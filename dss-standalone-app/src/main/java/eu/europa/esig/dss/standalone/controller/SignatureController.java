...
public class SignatureController implements Initializable {
	...
	private final String nbCertificatesTest = "Número de certificados fiables:  ";
		...
		signButton.setOnAction(new EventHandler<>() {
			...
			public void handle(ActionEvent event) {
				...
				service.setOnFailed(new EventHandler<>() {
					@Override
					public void handle(WorkerStateEvent event) {
						String errorMessage = "Se ha producido un error: " + service.getMessage();
	...
	private void save(DSSDocument signedDocument) {
		...
		if (fileToSave != null) {
			try (FileOutputStream fos = new FileOutputStream(fileToSave)) {
				...
				Alert alert = new Alert(AlertType.ERROR, "Error al guardar el fichero: " + e.getMessage(), ButtonType.CLOSE);
				alert.showAndWait();
			}
		}
	}
